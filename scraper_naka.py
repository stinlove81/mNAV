...
def clean_num_first(text):
    """문자열에 숫자가 여러 개일 경우 가장 첫 번째 숫자만 추출 (NAKA 주가용)"""
...
def run_naka_engine():
    url = "https://nakamoto.com/dashboard"
    ...
    try:
        ...
        # --- [데이터 추출] ---
        # 8번에서 첫 번째 숫자인 0.3764 추출 (주가)
        naka_price = clean_num_first(get_by_key(8))
        # 20번에서 mNAV 추출
        naka_mnav = clean_num(get_by_key(20))

        # 사장님 웹사이트 인자 명칭: "naka price", "naka mnav"
        update_data = {
            "naka price": naka_price,
            "naka mnav": naka_mnav
        }
        ...
        if naka_price > 0:
            ...
            print(f"✅ NAKA 업데이트 완료: {naka_price}$ / {naka_mnav}x")
        else:
            print("🚨 데이터를 찾지 못했습니다. 번호가 바뀌었는지 혹은 로딩 문제인지 확인 필요.")
